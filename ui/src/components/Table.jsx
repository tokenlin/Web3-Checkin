...
function Table() {
    ...
    const { isLoading, transactions, myTransactions, onSignIn } = useContext(TransactionContext)
    ...
    async function handleCheckInConfirm() {
        setOpen(false)
        if (position) {
            console.log("handleCheckInConfirm", position)
            console.log(detail.sn)
            console.log(parseInt(position.lng * 1000000))
            console.log(parseInt(position.lat * 1000000))
        ...
        <>
            <div className="table-gradient">
                ...
                >
                    ...
                    <div className="min-h-52">
                        <ul className={`${tableClass} border-b-2 border-[#302D2E]`}>
                            {["SN", "Description", "City", "Longitude", "Latitude", "StartTime", "EndTime"].map(li => (
                                <li key={li}>{li}</li>
                            ))}
                        </ul>
                ...
                <div className="flex w-full justify-center items-center flex-col md:p-14 py-10 px-4 text-white">
                    ...
                    <div className="min-h-52">
                        <ul className={`${tableClass} border-b-2 border-[#302D2E]`}>
                            {["SN", "Description", "City", "Longitude", "Latitude", "StartTime", "EndTime", "Action"].map(li => (
                                <li key={li}>{li} </li>
                            ))}
                        </ul>
